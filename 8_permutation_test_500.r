...
centre_500 <- read.csv('closest500.csv', header = FALSE)
centre_pros <- read.csv('num_shared_gene_top500.csv', header = TRUE)
...
for (i in 1:100) {
  ...
  sampled_genes <- sample(alltree2041$tree, 500)
  ...
  num_centre_genes <- sum(sampled_genes %in% centre_500$V1)
...
# Sort the dataframe by selection_count in descending order and select the top 500
top_500_genes <- gene_selection_data %>%
  arrange(desc(selection_count)) %>%
  head(500)

#select the top 500 genes with most frequency
randomness_MAX_500 <- top_500_genes$gene

#save
output <- paste0(csv_dir, "randomness_MAX_500.csv")
#write.csv(randomness_MAX_500, file = output)
...
generate_desaturated_colors <- function(n, saturation = 1.0) {
  ...
  colors <- hcl(h = hues, c = saturation * 100, l = 70)
  return(colors)
}
...
output <- paste0(fig_dir, "Permutation_test_500.pdf")
...
r <- hist(selection_frequencies, breaks = 30, 
          ...
          xlim = c(min(selection_frequencies, centre_pros$central_gene) - 30, 
                   max(selection_frequencies, centre_pros$central_gene) + 10))
